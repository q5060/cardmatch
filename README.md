...
1. 登入後進入 **對戰** 頁。
2. **搜尋欄**：輸入卡店名稱、地址或地標，地圖會移動到該處；選外部地點後可按「在此發布約戰」。地點搜尋使用 [Nominatim](https://nominatim.openstreetmap.org/)（OpenStreetMap，需網路）。
3. **藍色釘（卡店）**：點擊進入店家大廳，查看在該店打牌的玩家，或按「發布約戰公告」發布公告。
4. **綠色釘（自選地點）**：點擊地圖空白處發布，可填寫**約戰說明**（打什麼、程度）與**方便時段**；或點擊他人玩家釘查看需求（約 4 小時有效）。
5. **點擊釘或從大廳選擇玩家** 可 **發起約戰**。
6. 對方接受後，雙方按「我準備好了」即開始對戰；可於頁面內聊天並在賽後填寫戰績。

...
export default async function BattlePage({
  ...
  const [matchPayload, announcements, myAnnouncement, shops, userPrefs, queueStatus] =
    await Promise.all([
      viewerId ? fetchActiveMatchPayload(viewerId) : Promise.resolve(null),
  ...
  return (
    <div className="space-y-4">
      <BattleClient
        ...
        activeMatch={matchPayload?.activeMatch ?? null}
        defaultLat={defaultLat}
        defaultLng={defaultLng}
        defaultShopId={userPrefs?.defaultShopId ?? null}
        initialQueueStatus={queueStatus}
        battleResult={matchPayload?.battleResult ?? null}
